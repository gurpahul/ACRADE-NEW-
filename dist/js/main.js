...
var live = document.querySelector('#live'); // span class live is stored in variable

var Enemy = function Enemy(x, y, z) {
    // enemy constructor is made with properties
    this.x = x;
    this.y = y;
    this.z = z;
...
Enemy.prototype.update = function (dt) {
    //enemy prototpe has a function update        
    this.x += this.z * player.level * dt; //  to increase the speed
    if (this.x > 505) {
        this.x = -100;
    }
    if (this.x + 80 > player.x && this.x < player.x + 80 && this.y + 80 > player.y && this.y < player.y + 80) {
        // condtion that perform collision
        ...
        if (live.textContent == 0) {
            // to drease speed if game is over
            palyer.level = 0;
        }
    }
};
...
var Player = function Player(x, y, z) {
    // player constructor
    this.x = x;
    this.y = y;
    this.z = z;
...
Player.prototype.update = function () {
    if (this.x > 580) {
        // condition that palyer donot goes out of canvas
        this.x = -90;
    }
    if (this.y < 50) {
...
var enemy1 = new Enemy(0, 60, 80); // new instace of enemy is made
var enemy2 = new Enemy(0, 140, 120); // new instace of enemy is made
var enemy3 = new Enemy(0, 220, 180); // new instace of enemy is made

var allEnemies = [enemy1, enemy2, enemy3]; // stored in variable

var player = new Player(220, 400); // new palyer instaces is made

Player.prototype.handleInput = function (direction) {
    if (direction == "left" && this.x > 50) {
        /// consditions that our player move with keyboard keys
        this.x = this.x - 100;
